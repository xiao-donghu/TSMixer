...
class Exp_Long_Term_Forecast_a(Exp_Basic):
    def __init__(self, args):
        super(Exp_Long_Term_Forecast_a, self).__init__(args)
    ...
    def predict(self, setting, load=False):
        ...
        # 读取现有的时间序列 CSV 文件
        history_csv_path = './dataset/Overall/Overall_re.csv'  # 你的历史数据文件路径
        df_history = pd.read_csv(history_csv_path)

        # 假设历史数据的时间戳在一列中，并且未来时间间隔为 10 分钟
        last_timestamp = pd.to_datetime(df_history['date'].iloc[-1])  # 获取最后一个时间戳
        time_interval = 10  # 这里假设预测是每 60 分钟进行的
        ...
        # 将预测结果分割成两个 DataFrame
        num_predictions = preds_flattened.shape[1]
        half_num_predictions = num_predictions // 2

        df_preds_first_half = df_preds.iloc[:, :half_num_predictions].copy()  # 前半部分预测列
        df_preds_second_half = df_preds.iloc[:, half_num_predictions:].copy()  # 后半部分预测列
        ...
        # 从时间戳中提取年份
        df_preds_first_half['year'] = df_preds_first_half['time'].apply(lambda x: x.year)
        df_preds_second_half['year'] = df_preds_second_half['time'].apply(lambda x: x.year)

        df_preds_first_half.columns = ['y_pred_in', 'y_pred_out', 'time','year']
        df_preds_second_half.columns = ['y_pred_in', 'y_pred_out', 'time','year']

        def find_max_hourly_flow(df_preds_first_half, df_preds_second_half):
            """
            汇总10分钟粒度的预测数据到小时粒度，并找到当天进流量和出流量最高的小时。

            参数:
                df_preds_first_half (pd.DataFrame): 第一个预测DataFrame，包含 'time', 'y_pred_in', 'y_pred_out' 列。
                df_preds_second_half (pd.DataFrame): 第二个预测DataFrame，包含 'time', 'y_pred_in', 'y_pred_out' 列。

            返回:
                tuple: 包含四个时间戳，分别是
                       max_in_hour_first_half - 第一个DataFrame的进流量最高的小时的时间戳
                       max_out_hour_first_half - 第一个DataFrame的出流量最高的小时的时间戳
                       max_in_hour_second_half - 第二个DataFrame的进流量最高的小时的时间戳
                       max_out_hour_second_half - 第二个DataFrame的出流量最高的小时的时间戳
            """
            ...
            # 计算每小时的进出流量总和
            df_hourly_first_half = df_preds_first_half.groupby(pd.Grouper(key='time', freq='H')).agg({
                'y_pred_in': 'sum',
                'y_pred_out': 'sum'
            }).reset_index()

            df_hourly_second_half = df_preds_second_half.groupby(pd.Grouper(key='time', freq='H')).agg({
                'y_pred_in': 'sum',
                'y_pred_out': 'sum'
            }).reset_index()

            # 找到进流量最高的小时
            max_in_hour_first_half = df_hourly_first_half.loc[
                df_hourly_first_half['y_pred_in'].idxmax(), 'time']
            max_in_hour_second_half = df_hourly_second_half.loc[
                df_hourly_second_half['y_pred_in'].idxmax(), 'time']

            # 找到出流量最高的小时
            max_out_hour_first_half = df_hourly_first_half.loc[
                df_hourly_first_half['y_pred_out'].idxmax(), 'time']
            max_out_hour_second_half = df_hourly_second_half.loc[
                df_hourly_second_half['y_pred_out'].idxmax(), 'time']

            return max_in_hour_first_half, max_out_hour_first_half, max_in_hour_second_half, max_out_hour_second_half

        df_max_in_first_half, df_max_out_first_half, df_max_in_second_half, df_max_out_second_half = find_max_hourly_flow(
            df_preds_first_half, df_preds_second_half)
        print(df_max_in_first_half)
        
        df_preds_first_half['in_high_time'] = df_max_in_first_half
        df_preds_first_half['out_high_time'] = df_max_out_first_half
        df_preds_second_half['in_high_time'] = df_max_in_second_half
        df_preds_second_half['out_high_time'] = df_max_out_second_half
        print(df_preds_first_half.shape)


        # 添加 location 列
        df_preds_first_half['location'] = '舟山西'
        df_preds_second_half['location'] = '朱家尖'
        ...
        def add_date_range(df):
            def get_date_range(row):
                ...
                if date in cn_holidays:
                    ...
                    next_day = row['time'] + pd.DateOffset(days=1)
        ...
        # 示例使用
        # 假设df_preds_second_half和df_preds_first_half已经被定义并且包含了'time'列
        add_date_range(df_preds_second_half)
        add_date_range(df_preds_first_half)

        # 重命名预测列
        df_preds_first_half.columns = ['y_pred_in', 'y_pred_out', 'time','year','in_high_time','out_high_time',
                                       'location','date_range','date_type','record_time']
        df_preds_second_half.columns = ['y_pred_in', 'y_pred_out', 'time','year','in_high_time','out_high_time',
                                        'location','date_range','date_type','record_time']

        new_column_order = ['date_range', 'time', 'date_type','year','location', 'y_pred_in', 'y_pred_out', 'in_high_time',
                            'out_high_time', 'record_time']
       ...
       # 对于第一个数据帧
        ...
        # 将第二个 DataFrame 追加到第一个 DataFrame 后面，并去掉第二个 DataFrame 的标题行
        predicted_data_a = pd.concat([df_preds_first_half, df_preds_second_half], ignore_index=True)

        ############################################生成第二个dataframe###############################################################

        df_preds_first_half = df_preds.iloc[:, :1].copy()  # 第一列部分预测列
        df_preds_second_half = df_preds.iloc[:, 1:2].copy()  # 第二列预测列
        df_preds_third_half = df_preds.iloc[:, 2:3].copy()
        df_preds_fourth_half = df_preds.iloc[:, 3:4].copy()


        df_preds_first_half['time'] = np.tile(future_timestamps, preds.shape[0])  # 重复时间戳
        df_preds_second_half['time'] = np.tile(future_timestamps, preds.shape[0])
        df_preds_third_half['time'] = np.tile(future_timestamps, preds.shape[0])
        df_preds_fourth_half['time'] = np.tile(future_timestamps, preds.shape[0])

        df_preds_first_half['time'] = pd.to_datetime(df_preds_first_half['time'])
        df_preds_second_half['time'] = pd.to_datetime(df_preds_second_half['time'])
        df_preds_third_half['time'] = pd.to_datetime(df_preds_third_half['time'])
        df_preds_fourth_half['time'] = pd.to_datetime(df_preds_fourth_half['time'])
        ...
        add_date_type(df_preds_third_half)
        add_date_type(df_preds_fourth_half)
        ...
        df_preds_third_half['date'] = df_preds_third_half['time'].apply(extract_date)
        df_preds_fourth_half['date'] = df_preds_fourth_half['time'].apply(extract_date)
        df_preds_first_half['location'] ='舟山西'
        df_preds_second_half['location'] = '舟山西'
        df_preds_third_half['location'] = '朱家尖'
        df_preds_fourth_half['location'] = '朱家尖'

        df_preds_first_half['direction'] = 'in'
        df_preds_second_half['direction'] = 'out'
        df_preds_third_half['direction'] = 'in'
        df_preds_fourth_half['direction'] = 'out'

        df_preds_first_half['model_type'] = '实战预测a'
        df_preds_second_half['model_type'] = '实战预测a'
        df_preds_third_half['model_type'] = '实战预测a'
        df_preds_fourth_half['model_type'] = '实战预测a'

        df_preds_first_half['model_name'] = 'TSMixer'
        df_preds_second_half['model_name'] = 'TSMixer'
        df_preds_third_half['model_name'] = 'TSMixer'
        df_preds_fourth_half['model_name'] = 'TSMixer'
        ...
        df_preds_second_half.columns = ['y_pred', 'time', 'date_type', 'date', 'location', 'direction', 'model_type',
                                      'model_name']
        df_preds_third_half.columns = ['y_pred', 'time', 'date_type', 'date', 'location', 'direction', 'model_type','model_name']
        df_preds_fourth_half.columns = ['y_pred', 'time', 'date_type', 'date', 'location', 'direction', 'model_type','model_name']

        new_column_order = ['date', 'time', 'date_type', 'location', 'direction', 'model_type', 'model_name','y_pred']

        df_preds_first_half = df_preds_first_half.reindex(columns=new_column_order)
        df_preds_second_half = df_preds_second_half.reindex(columns=new_column_order)
        df_preds_third_half = df_preds_third_half.reindex(columns=new_column_order)
        df_preds_fourth_half = df_preds_fourth_half.reindex(columns=new_column_order)
        # 将后面的三个DataFrame追加到第一个DataFrame中
        predicted_data_a_1 = pd.concat([df_preds_first_half, df_preds_second_half, df_preds_third_half, df_preds_fourth_half],
                                ignore_index=True)


        return predicted_data_a,predicted_data_a_1