...
class Exp_Long_Term_Forecast_jintang_b_hour(Exp_Basic):
    def __init__(self, args):
        super(Exp_Long_Term_Forecast_jintang_b_hour, self).__init__(args)
    ...
    def predict(self, setting, load=False):
        ...
        # 读取现有的时间序列 CSV 文件
        history_csv_path = './dataset/Overall/Overall_jintang_hour.csv'  # 你的历史数据文件路径
        ...
        df_preds_hour = df_preds.copy()  # 前半部分预测列


        df_preds_hour['start_time'] = np.tile(future_timestamps, preds.shape[0])  # 重复时间戳



        df_preds_hour['start_time'] = pd.to_datetime(df_preds_hour['start_time'])

        # 从时间戳中提取年份
        df_preds_hour['year'] = df_preds_hour['start_time'].apply(lambda x: x.year)

        # 计算 end_time 列
        df_preds_hour['end_time'] = pd.to_datetime(df_preds_hour['start_time']) + pd.Timedelta(
            minutes=time_interval)

        # 添加 pred_time 列（时间间隔）
        df_preds_hour['pred_time'] = time_interval


        # 添加 location 列
        df_preds_hour['location'] = '金塘'
        ...
        add_date_range(df_preds_hour)

        # 重命名预测列
        df_preds_hour.columns = ['y_pred_in', 'y_pred_out', 'start_time','year', 'end_time', 'pred_time',
                                       'location', 'date_range','date_type','record_time']

        print(df_preds_hour.head())
        new_column_order = ['date_range','year', 'date_type', 'location', 'start_time', 'end_time', 'pred_time', 'y_pred_in',
                            'y_pred_out','record_time']

        # 对于第一个数据帧
        df_preds_hour = df_preds_hour.reindex(columns=new_column_order)



        predicted_data_b_hour = df_preds_hour
#####################################################生成第二个dataframe#########################################
        ...
        predicted_data_b_1_hour = pd.concat([df_preds_first_half, df_preds_second_half],
                                ignore_index=True)
