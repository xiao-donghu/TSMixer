...
class Exp_Long_Term_Forecast_b_hour(Exp_Basic):
    def __init__(self, args):
        super(Exp_Long_Term_Forecast_b_hour, self).__init__(args)
    ...
    def predict(self, setting, load=False):
        ...
        # 读取现有的时间序列 CSV 文件
        history_csv_path = './dataset/Overall/Overall_merge_hour.csv'  # 你的历史数据文件路径
        df_history = pd.read_csv(history_csv_path)

        # 假设历史数据的时间戳在一列中，并且未来时间间隔为 60 分钟
        last_timestamp = pd.to_datetime(df_history['date'].iloc[-1])  # 获取最后一个时间戳
        time_interval = 60  # 这里假设预测是每 60 分钟进行的
        ...
        df_preds_first_half['start_time'] = np.tile(future_timestamps, preds.shape[0])  # 重复时间戳
        df_preds_second_half['start_time'] = np.tile(future_timestamps, preds.shape[0])


        df_preds_first_half['start_time'] = pd.to_datetime(df_preds_first_half['start_time'])
        df_preds_second_half['start_time'] = pd.to_datetime(df_preds_second_half['start_time'])
        # 从时间戳中提取年份
        df_preds_first_half['year'] = df_preds_first_half['start_time'].apply(lambda x: x.year)
        df_preds_second_half['year'] = df_preds_second_half['start_time'].apply(lambda x: x.year)
        # 计算 end_time 列
        df_preds_first_half['end_time'] = pd.to_datetime(df_preds_first_half['start_time']) + pd.Timedelta(
            minutes=time_interval)
        df_preds_second_half['end_time'] = pd.to_datetime(df_preds_second_half['start_time']) + pd.Timedelta(
            minutes=time_interval)

        # 添加 pred_time 列（时间间隔）
        df_preds_first_half['pred_time'] = time_interval
        df_preds_second_half['pred_time'] = time_interval
        ...
        # 重命名预测列
        df_preds_first_half.columns = ['y_pred_in', 'y_pred_out', 'start_time','year', 'end_time', 'pred_time',
                                       'location', 'date_range','date_type','record_time']
        df_preds_second_half.columns = ['y_pred_in', 'y_pred_out', 'start_time','year', 'end_time', 'pred_time',
                                        'location', 'date_range','date_type','record_time']
        print(df_preds_first_half.head())
        new_column_order = ['date_range','year', 'date_type', 'location', 'start_time', 'end_time', 'pred_time', 'y_pred_in',
        ...
        predicted_data_b_hour = pd.concat([df_preds_first_half, df_preds_second_half], ignore_index=True)
#####################################################生成第二个dataframe#########################################
        ...
        df_preds_first_half['model_type'] = '实战预测b_hour'
        df_preds_second_half['model_type'] = '实战预测b_hour'
        df_preds_third_half['model_type'] = '实战预测b_hour'
        df_preds_fourth_half['model_type'] = '实战预测b_hour'
        ...
        df_preds_second_half.columns = ['y_pred', 'time', 'date_type', 'date', 'location', 'direction', 'model_type',
                                        'model_name']
        df_preds_third_half.columns = ['y_pred', 'time', 'date_type', 'date', 'location', 'direction', 'model_type',
                                       'model_name']
        df_preds_fourth_half.columns = ['y_pred', 'time', 'date_type', 'date', 'location', 'direction', 'model_type',
                                        'model_name']
        ...
        predicted_data_b_1_hour = pd.concat([df_preds_first_half, df_preds_second_half, df_preds_third_half, df_preds_fourth_half],
                                ignore_index=True)


        return predicted_data_b_hour,predicted_data_b_1_hour