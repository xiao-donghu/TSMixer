...
class Exp_Long_Term_Forecast_b(Exp_Basic):
    def __init__(self, args):
        super(Exp_Long_Term_Forecast_b, self).__init__(args)
    ...
    def predict(self, setting, load=False):
        ...
        # 读取现有的时间序列 CSV 文件
        history_csv_path = './dataset/Overall/Overall_merge_30min.csv'  # 你的历史数据文件路径
        ...
        predicted_data_b = pd.concat([df_preds_first_half, df_preds_second_half], ignore_index=True)
        #############################################
        # 读取现有的时间序列 CSV 文件

        df_preds_first_half = df_preds.iloc[:, :1].copy()  # 第一个预测列
        ...
        df_preds_third_half['model_type'] = '实战预测b'
        df_preds_fourth_half['model_type'] = '实战预测b'
        ...
        predicted_data_b_1 = pd.concat(
            [df_preds_first_half, df_preds_second_half, df_preds_third_half, df_preds_fourth_half],
            ignore_index=True)
