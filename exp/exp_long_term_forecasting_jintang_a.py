...
class Exp_Long_Term_Forecast_jintang_a(Exp_Basic):
    def __init__(self, args):
        super(Exp_Long_Term_Forecast_jintang_a, self).__init__(args)
    ...
    def predict(self, setting, load=False):
        ...
        # 读取现有的时间序列 CSV 文件
        history_csv_path = './dataset/Overall/Overall_jintang_re.csv'  # 你的历史数据文件路径
        ...
        time_interval = 10  # 这里假设预测是每 10 分钟进行的
        ...
        df_preds_jintang = df_preds.copy()  # 前半部分预测列


        df_preds_jintang['time'] = np.tile(future_timestamps, preds.shape[0])
        df_preds_jintang['time'] = pd.to_datetime(df_preds_jintang['time'])


        # 从时间戳中提取年份
        df_preds_jintang['year'] = df_preds_jintang['time'].apply(lambda x: x.year)

        df_preds_jintang.columns = ['y_pred_in', 'y_pred_out', 'time', 'year']


        def find_max_hourly_flow(df_preds_first_half):
            """
            ...
            return max_in_hour_first_half, max_out_hour_first_half


        df_max_in, df_max_out = find_max_hourly_flow(
            df_preds_jintang)
        print(df_max_in)

        df_preds_jintang['in_high_time'] = df_max_in
        df_preds_jintang['out_high_time'] = df_max_out

        print(df_preds_jintang.shape)

        # 添加 location 列
        df_preds_jintang['location'] = '金塘'
        ...
        add_date_range(df_preds_jintang)

        # 重命名预测列
        df_preds_jintang.columns = ['y_pred_in', 'y_pred_out', 'time', 'year', 'in_high_time', 'out_high_time',
                                       'location', 'date_range', 'date_type', 'record_time']

        new_column_order = ['date_range', 'time', 'date_type', 'year', 'location', 'y_pred_in', 'y_pred_out',
                            'in_high_time',
                            'out_high_time', 'record_time']

        # 对于第一个数据帧
        df_preds_jintang = df_preds_jintang.reindex(columns=new_column_order)

        # 将第二个 DataFrame 追加到第一个 DataFrame 后面，并去掉第二个 DataFrame 的标题行
        predicted_data_a = df_preds_jintang
