...
class MambaBlock(nn.Module):
    ...
    def selective_scan(self, u, delta, A, B, C, D):
        ...
        deltaA = torch.exp(np.einsum(delta, A, "b l d, d n -> b l d n")) # A is discretized using zero-order hold (ZOH) discretization
        deltaB_u = np.einsum(delta, B, u, "b l d, b l n, b l d -> b l d n") # B is discretized using a simplified Euler discretization instead of ZOH. From a discussion with authors: "A is the more important term and the performance doesn't change much with the simplification on B"
        ...
        for i in range(l):
            ...
            y = np.einsum(x, C[:, i, :], "b d n, b n -> b d")
            ys.append(y)
